...
class Token(object):
    ...
    def __str__(self):
        ...
        if self.data_type == Macro.TOKEN_DATA_FUNC:
            return "%s (%s)" % (types[self.type - self.TOKEN_EOF], \
                                data_types[self.data_type - Macro.TOKEN_DATA_VOID])
...
class Block(object):
    ...
    def __str__(self):
        ...
        if self.name:
            return "%s name %s line %d" % \
                (types[self.type - self.INPUT_STRING], self.name, self.line)
        else:
